...
class S3Collector(object):
    ...
    def collect(self):
        ...
        latest_file_timestamp_gauge = GaugeMetricFamily(
            ...
            "Last modified timestamp(milliseconds) for latest file in " "folder",
            labels=["folder", "file"],
        )
        oldest_file_timestamp_gauge = GaugeMetricFamily(
            "s3_oldest_file_timestamp",
            "Last modified timestamp(milliseconds) for oldest file in " "folder",
            labels=["folder", "file"],
        )
        ...
        for folder in config.get("folders"):
            ...
            logging.debug(
                'Listing contents of bucket: "{0}" with prefix: "{1}"'.format(
                    bucket, prefix
                )
            )
            token = None
            ...
            while True:
                try:
                    kw = {"Bucket": bucket, "Delimiter": delimiter}
                    ...
                    success = False
                    logging.error(
                        'Error listing contents of bucket: "{0}" with prefix: "{1}" error: "{2}"'.format(
                            bucket, prefix, err
                        )
                    )
                    continue
                ...
                if "Contents" in result:
                    ...
                    success = False
                    logging.error(
                        'Error no content found in bucket: "{0}" with prefix: "{1}"'.format(
                            bucket, prefix
                        )
                    )
                    break
            for pattern in patterns:
                ...
                file_count_gauge.add_metric([folder, last_file_name], len(files))

                latest_file_timestamp_gauge.add_metric(
                    [folder, last_file_name], latest_modified
                )
                oldest_file_timestamp_gauge.add_metric(
                    [folder, last_file_name], oldest_modified
                )
                latest_file_size_gauge.add_metric(
                    [folder, last_file_name], int(last_file["Size"])
                )
                oldest_file_size_gauge.add_metric(
                    [folder, oldest_file_name], int(oldest_file["Size"])
                )

                success_gauge.add_metric([folder], int(success))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Expose metrics for S3 storage")
    parser.add_argument(
        "config_file_path",
        nargs="?",
